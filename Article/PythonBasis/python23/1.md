...
各位有没有过这种崩溃时刻——两点水把代码打包发给同事，同事跑了一下，报一堆 `ModuleNotFoundError`。两点水说：「你 `pip install` 一下就好了。」同事问：「装哪个版本？」两点水翻了翻自己的电脑，半天没找到一份完整的 `requirements.txt`，最后只好憋出一句：「呃，我电脑能跑啊，奇了怪了。」
...
声明这个项目需要哪个版本的 `Python`。两点水强烈建议各位都写上这一行，原因有三：
...
2026 年开新项目，两点水的建议：直接写 `>=3.10` 或 `>=3.11`。3.9 已经接近退役，3.10 起才有现代特性（`match` 语句、更好的类型提示）。
...
但两点水更推荐独立安装——`uv` 本来就是为了脱离 `pip` 而设计的，没必要绑着 `pip` 装。
...
激活之后跟传统流程一样。但两点水自己几乎不激活了，`uv run` 加在每条命令前面就够。
...
「精确版本」是关键。`uv.lock` 里记录的不是 `httpx>=0.28`，而是 `httpx==0.28.1`、加上 `httpx` 的所有传递依赖也都钉到具体版本。所以 `uv sync` 出来的环境，跟两点水电脑上的环境，**字节级一模一样**。
...
激活之后用 `python` 命令直接跑，不需要 `uv run` 前缀。两点水偶尔在「连续跑很多命令、要保持环境激活」时这么用。
...
各位经常听人说哪个工具好哪个工具坏。两点水做个不带感情色彩的对比：
