...
print(make_user(name='水哥', dept='做鸭事业部'))
```
...
print(make_profile('水哥', 28, 12000.5, True))
```

输出结果：

```
水哥（28 岁，工资 12000.5，在职：True）
```
...
```
水哥 / 28 岁 / 1.75m / 实习：False / 技能：['Python', 'Django'] / 地址：{'city': '深圳', 'street': '科技园'}
```
...
```
2026-04-28：昵称：水哥  部门：做鸭事业部 上班打卡成功
```
...
print(total_punches(punches, '水哥'))
...
db: dict[str, list[tuple[str, list[str]]]] = {}
add_employee(db, '做鸭事业部', ('水哥', ['Python', '后端']))
...
db: EmployeeDB = {}
add_employee(db, '做鸭事业部', ('水哥', ['Python']))
print(db)
```
