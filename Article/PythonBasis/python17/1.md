...
童鞋看着这个 `calc` 一脸懵：`x` 和 `y` 到底是数字还是字符串？`op` 还能传啥？返回的是啥类型？只能跑去翻调用方的代码，或者直接喊「两点水，这函数到底咋用啊」。
...
各位想想，假如有这么一个场景：「两点水」周末临时请假，把手头的项目交接给童鞋。代码里有几十个函数，每个函数都长成 `def process(data, config, cb)` 这副样子。童鞋打开一个文件就懵了——`data` 是字典还是列表？`config` 是对象还是路径？`cb` 是函数还是字符串？每一处都得跑去问，要么就翻一遍调用栈。这一个周末，怕是整个人都得搭进去。
...
那么有童鞋就要问了：「变量赋值的时候，类型不是一眼就能看出来吗？我直接 `name = '两点水'` 不就完了？还加注解干啥？」
...
来一个非常贴近实际的小场景。我们要写一个登录积分系统的初始化代码——「两点水」每天登录会累积一些积分，但是初始时还没积分：
...
跑 `python bad.py`，能跑出来（虽然结果是 `两点水两点水两点水`），但是跑 `mypy bad.py`：
