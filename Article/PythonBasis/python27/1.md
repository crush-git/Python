...
两点水自己在本地写了一个小工具，比如「批量重命名截图文件」、「统计微信导出的聊天记录」、「自动给图片加水印」。脚本一两百行，跑起来挺顺手，自己天天用。然后某天一个朋友看见了，说：「这玩意儿挺好啊，能给我一份吗？」

两点水于是把 `tool.py` 用微信发过去。朋友收到之后，第一句话是——「这怎么用？」
...
两点水的建议是——为了不让用户搞晕，**两个名字尽量保持一致**。比如包名叫 `my-tool`，导入名就叫 `my_tool`（连字符变下划线，Python 圈通用习惯）。
...
把你的工具装到自己的环境里。两点水写到这里都觉得有点小激动——这意味着你的代码已经在全球公共仓库上了，谁都能用上。
...
两点水的建议是——**新项目用自动法**。一开始用手动法没问题，但是到了第三、第四个版本，开发者多起来的时候，自动法能省掉很多对帐的功夫。
...
社区惯例——工具/库类项目用 MIT 或 Apache 2.0，应用类项目自己拿主意。两点水的小工具一律 MIT，省事。
...
两点水强烈推荐 2026 年的新项目一上来就用可信发布，比 token 干净太多。
...
各位老在「位置参数」和「选项参数」之间纠结，两点水给一个一句话原则——
...
各位跟着两点水写一个完整的 `todo-cli` 工具，最后发到 PyPI（演练版本发到 TestPyPI）。这个工具支持三个命令——
...
打开 `https://pypi.org/project/todo-cli/`，自己的小工具上线了。两点水写到这里又有点小激动——这就是「全世界一行 `pip install todo-cli` 就能用」的感觉。
