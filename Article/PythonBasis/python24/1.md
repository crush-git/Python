...
六个工具，六份配置文件，六套规则，每个工具自己装一遍，CI 上每个跑一遍，commit 之前每个调一遍。两点水当年配一个新项目的 lint 流水线，光研究这些工具怎么互相不打架就能花掉半天。
...
各位读到这里可能怀疑：「真的有这么快吗，会不会是数据造假？」两点水亲自在自己电脑上跑过，结论是**真的就是这么快**。一个一千文件的项目，`ruff check .` 经常一秒之内出结果，肉眼看不到「跑」的过程。
