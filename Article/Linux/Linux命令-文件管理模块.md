...
> 注：本章原 12 张截图（七牛云 bkt.clouddn.com）已失效，2026-04-28 根据上下文按主流 Linux 命令用法重写。
...
```
test.txt  demo.sh  hello.py
.  ..  .bashrc  .profile  test.txt  demo.sh  hello.py
total 12K
-rw-r--r-- 1 user user  128 Apr 28 10:00 demo.sh
-rw-r--r-- 1 user user   64 Apr 28 10:01 hello.py
-rw-r--r-- 1 user user  256 Apr 28 10:02 test.txt
```
...
```
rm: remove regular file 'hello.py'? y
```
...
```bash
ln -s /home/walter/source.txt link_soft.txt
ln /home/walter/source.txt link_hard.txt
ls -lh
```

```
lrwxrwxrwx 1 user user   24 Apr 28 10:10 link_soft.txt -> /home/walter/source.txt
-rw-r--r-- 2 user user  128 Apr 28 10:10 link_hard.txt
-rw-r--r-- 2 user user  128 Apr 28 10:10 source.txt
```
...
grep -in 'HELLO' demo.txt
grep -v 'hello' demo.txt
```
